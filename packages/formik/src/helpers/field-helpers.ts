...
export const selectFieldOnChange = <Value, Values>(
  ...
  nameOrConfig?: string | FieldHookConfig<Value, Values>
) => {
  return (
    ...
    const {
      ...
      parse: configParse,
      multiple,
    }: FieldHookConfig<Value, Values> = isObject(nameOrConfig)
      ? nameOrConfig
